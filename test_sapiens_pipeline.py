...
print(f"   -> Temps: {t1-t0:.1f}s ({(t1-t0)/n:.2f}s/frame)\n")
...
print(f"   -> Angles valides: {non_none}/{n}\n")
...
# 5. CYCLES + ANALYSE
if total_ev >= 4:
    print("5. Cycles + Analyse...")
    try:
        cycles = mg.segment_cycles(data)
        stats = mg.analyze_gait(data, cycles)
        sp = stats["spatiotemporal"]
        print(f"   -> {len(cycles['cycles'])} cycles, cadence={sp['cadence_steps_per_min']:.1f} pas/min")
    except Exception as e:
        print(f"   -> Pas assez de cycles: {e}")
        cycles, stats = None, None
else:
    print("5. Pas assez d'events pour les cycles (60 frames = ~1s)")
    cycles, stats = None, None
...
print(f"\n=== SAPIENS COMPLET EN {t_total:.1f}s ===")
print("Sapiens fonctionne de bout en bout (sans sapiens_inference).")
