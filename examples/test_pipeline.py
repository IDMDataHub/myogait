...
print("=== COMPLETE PIPELINE myogait v" + mg.__version__ + " ===\n")
...
# 2. NORMALIZATION
print("2. Normalization (butterworth + pixel_ratio + center)...")
...
# 3. ANGLES
print("3. Angle computation (sagittal_vertical_axis)...")
...
lto = len(ev["left_to"])
rto = len(ev["right_to"])
print(f"   -> HS left: {lhs}, HS right: {rhs}")
print(f"   -> TO left: {lto}, TO right: {rto}")
...
for side in ["left", "right"]:
    if side in cycles.get("summary", {}):
        ...
        print(f"   -> {side}: {nn} cycles, mean/std computed")
print()

# 6. ANALYSIS
print("6. Gait analysis...")
stats = mg.analyze_gait(data, cycles)
sp = stats["spatiotemporal"]
print(f"   -> Cadence: {sp['cadence_steps_per_min']:.1f} steps/min")
print(f"   -> Stride duration: {sp['stride_time_mean_s']:.3f} +/- {sp['stride_time_std_s']:.3f} s")
...
print(f"   -> Hip ROM symmetry: {hip_si}")
var = stats.get("variability", {})
cv_val = var.get("cycle_duration_cv", "N/A")
print(f"   -> Cycle variability: CV={cv_val}")
...
# 7. VALIDATION
print("7. Biomechanical validation...")
...
print(f"=== COMPLETE PIPELINE IN {t_total:.1f}s ===")
print("Everything works end-to-end.")
